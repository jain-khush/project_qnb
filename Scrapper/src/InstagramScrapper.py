...
class InstagramScrapper:
    ...
    def Scrape_Instagram(self, tag, limit=20, browser='chrome'):
        ...
        msg = "Loading Images"
        class_div_img = ["_aagv"]
        #print(driver.find_element(By.NAME, class_div_img))

        for div in class_div_img:
            # print(driver.find_elements_by_class_name(div))
        ...
        img_caption = []
        hashtags = {}
        for data in driver.find_elements(By.TAG_NAME, 'img'):
            #     u.get_attribute("href").split("/")[4]
            img_caption.append(data.get_attribute("alt"))
            img_src.append(data.get_attribute("src"))

        img_caption = img_caption[11:limit + 11]
        img_src = img_src[11:limit + 11]
        img_caption.sort()
